# Environment Setup Guide for SynarkOS Contributors

Welcome to the SynarkOS development environment setup guide! This comprehensive guide will walk you through setting up your development environment from scratch, whether you're a first-time contributor or an experienced developer.
...
1. **Fork the repository** on GitHub: [github.com/synarkos/synarkos](https://github.com/synarkos/synarkos)

2. **Clone your fork**:
```bash
git clone https://github.com/YOUR_USERNAME/synarkos.git
cd synarkos
```

3. **Add upstream remote**:
```bash
git remote add upstream https://github.com/synarkos/synarkos.git
```

4. **Verify remotes**:
```bash
git remote -v
# origin    https://github.com/YOUR_USERNAME/synarkos.git (fetch)
# origin    https://github.com/YOUR_USERNAME/synarkos.git (push)
# upstream  https://github.com/synarkos/synarkos.git (fetch)
# upstream  https://github.com/synarkos/synarkos.git (push)
```
...
SynarkOS uses several tools to maintain code quality:
...
=== "Formatting"
    ...
    **Black** - Code formatter
    ```bash
    # Format code
    poetry run black synarkos/
    # or with pip:
    black synarkos/
    
    # Check formatting without making changes
    black synarkos/ --check --diff
    ```
...
=== "Linting"
    ...
    **Ruff** - Fast Python linter
    ```bash
    # Run linter
    poetry run ruff check synarkos/
    # or with pip:
    ruff check synarkos/
    
    # Auto-fix issues
    ruff check synarkos/ --fix
    ```
...
poetry run pytest --cov=synarkos tests/
...
=== "CLI"
    ```bash
    synarkos create-agent --name myagent
    ```
```
...
DATABASE_URL=sqlite:///synarkos.db
```
...
### 1. Basic Import Test
```bash
poetry run python -c "from synarkos import Agent; print('✅ Import successful')"
```
...
poetry run black synarkos/ --check
poetry run ruff check synarkos/
...
poetry run black synarkos/
poetry run ruff check synarkos/ --fix

# Run a quick smoke test
poetry run python -c "from synarkos import Agent; print('✅ All good')"
```
...
=== "Import Errors"
    ...
    **Problem**: Cannot import synarkos modules
    ```bash
...
2. **Explore the Codebase**: Start with `synarkos/structs/agent.py`
...
!!! success "You're Ready!"
    Your SynarkOS development environment is now set up! You're ready to contribute to the most important technology for multi-agent collaboration.
...
poetry run black synarkos/             # Format code
poetry run ruff check synarkos/        # Lint code
