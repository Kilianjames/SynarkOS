...
This guide shows you how to deploy your SynarkOS agents as REST APIs using FastAPI and Uvicorn. This is the fastest way to expose your agents via HTTP endpoints.
...
```bash
pip install fastapi uvicorn synarkos
```
...
app = FastAPI(
    ...
    description="REST API for SynarkOS agents",
    version="1.0.0"
)
...
async def root():
    ...
    return {"message": "SynarkOS Agent API is running!", "status": "healthy"}
