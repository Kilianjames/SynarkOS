...
out = agent.run(
    "Scrape synarkos.ai website and provide a full report of the company  does. The format type should be full."
)
print(out)
```
...
out = batched_grid_agent_execution(
    agents=[agent, agent],
    tasks=[
        "Scrape synarkos.ai website and provide a full report of the company's mission, products, and team. The format type should be full.",
