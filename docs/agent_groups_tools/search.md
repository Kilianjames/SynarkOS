...
This documentation covers the search tools available in the `synarkos-tools` package.
...
```python
from synarkos_tools.search import fetch_web_articles_bing_api

# Fetch articles about AI
results = fetch_web_articles_bing_api("synarkos ai github")
print(results)
```
...
```python
from synarkos_tools.search import search_exa_ai
...
```python
from synarkos_tools.search import WebsiteChecker
...
from synarkos_tools.search import search
...
For issues and feature requests, please visit the [GitHub repository](https://github.com/synarkos-tools). 