...
SwarmRearrange is a class for orchestrating multiple synarkos in a sequential or parallel flow pattern. It provides thread-safe operations for managing swarm execution, history tracking, and flow validation.

Full Path: `from synarkos.structs.swarm_rearrange import SwarmRearrange`
...
| description | str | "A swarm of synarkos..." | Description of the arrangement |
| synarkos | List[Any] | [] | List of swarm objects to be managed |
...
### remove_swarm(agent_group_name: str) 
Removes a swarm by name from the arrangement.

### add_synarkos(synarkos: List[Any])
Adds multiple synarkos to the arrangement.
...
```python
import os
from synarkos import Agent, AgentRearrange, SwarmRearrange
...
print(
    swarm_arrange.run(
        "Analyze synarkos, 150k revenue with 45m+ agents build, with 1.4m downloads since march 2024"
    )
)
