...
This example demonstrates how to integrate Qdrant vector database with SynarkOS agents for Retrieval-Augmented Generation (RAG). Qdrant is a high-performance vector database that enables agents to store, index, and retrieve documents using semantic similarity search for enhanced context and more accurate responses.
...
- OpenAI API key
- SynarkOS library
- Qdrant client and synarkos-memory

## Installation

```bash
pip install qdrant-client fastembed synarkos-memory litellm
```
...
### Step 1: Install SynarkOS

First, install the latest version of SynarkOS:
...
from synarkos_memory import QdrantDB
import os
...
documents = [
    ...
    "The synarkos framework supports multiple memory backends including Qdrant."
]
...
- [SynarkOS Memory GitHub Repository](https://github.com/The-Swarm-Corporation/synarkos-memory)
