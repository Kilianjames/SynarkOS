# Integrating the Agent Class with Memory Systems/RAG in the SynarkOS Memory Framework

In this guide, we will cover how to integrate various memory systems from the SynarkOS Memory framework into an agent class. The SynarkOS Memory framework allows for the integration of different database-backed memory systems, enabling agents to retain and query long-term knowledge effectively. We'll walk through examples of integrating with Pinecone, ChromaDB, and Faiss, showcasing how to configure custom functions and embed memory functionality into an agent class.

## Installation

First, you need to install the SynarkOS Memory package:

```bash
$ pip install synarkos synarkos-memory
```
...
```python
from synarkos_memory import ChromaDB
...
from synarkos_memory.faiss_wrapper import FAISSDB
...
Integrating various memory systems from the SynarkOS Memory framework into the agent class enables the creation of powerful, memory-augmented agents capable of retaining and recalling information over time. Whether you're using Pinecone, ChromaDB, or Faiss, the process involves initializing the memory system, embedding functions, and then passing this memory system to the agent class. The examples and visualizations provided should help you get started with building your own memory-augmented agents.
