...
!!! info "Overview"
    vLLM is a high-performance and easy-to-use library for LLM inference and serving. This guide explains how to integrate vLLM with SynarkOS for efficient, production-grade language model deployment.
...
=== "pip"
    ```bash
    pip install -U vllm synarkos
    ```

=== "poetry"
    ```bash
    poetry add vllm synarkos
    ```

## Basic Usage

Here's a simple example of how to use vLLM with SynarkOS:

```python title="basic_usage.py"
from synarkos.utils.vllm_wrapper import VLLMWrapper
...
You can easily integrate vLLM with SynarkOS agents for more complex workflows:
