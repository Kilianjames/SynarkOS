...
This guide demonstrates how to configure SynarkOS agents with custom base URLs and API keys to use models from various providers beyond the default configurations.

The SynarkOS framework supports over 600+ models through the `model_name` parameter, and you can easily integrate with any model provider by specifying custom `llm_base_url` and `llm_api_key` parameters. This flexibility allows you to use models from:
...
The SynarkOS framework supports models from these major providers through custom base URLs:
...
The custom base URL and API key configuration in SynarkOS provides incredible flexibility to use models from any provider. This approach allows you to:
