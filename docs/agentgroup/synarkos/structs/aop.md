...
The Agent Orchestration Protocol (AOP) is a powerful framework for deploying multiple SynarkOS agents as tools in an MCP (Model Context Protocol) server. This enables you to create a distributed system where agents can be accessed as individual tools, making them available for use by other systems, applications, or clients.
...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `agent` | `AgentType` | Required | The synarkos Agent instance to deploy |
...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `agents` | `List[Agent]` | Required | List of synarkos Agent instances |
...
| **Other SynarkOS**       | As part of larger multi-agent systems            |
