...
- **Router-based task distribution**: Central router agent analyzes incoming tasks and directs them to appropriate specialized synarkos

- **Hybrid architecture**: Combines hierarchical control with clustered specialization

- **Parallel processing**: Multiple synarkos can work simultaneously on different aspects of complex tasks

- **Flexible swarm types**: Supports both sequential and concurrent workflows within synarkos

- **Comprehensive result aggregation**: Collects and combines outputs from all contributing synarkos


## Diagram

The HHCS architecture follows a hierarchical structure with the router agent at the top level, specialized synarkos at the middle level, and individual agents at the bottom level.
...
| `synarkos` | List[SwarmRouter] | [] | List of available swarm routers |
...
| `find_swarm_by_name` | `agent_group_name` (str) | SwarmRouter | Retrieves a swarm by its name |
| `route_task` | `agent_group_name` (str), `task_description` (str) | None | Routes a task to a specific swarm |
| `get_synarkos_info` | None | str | Returns formatted information about all available synarkos |
