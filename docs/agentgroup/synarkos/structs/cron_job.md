...
A wrapper class that turns any callable (including SynarkOS agents) into a scheduled cron job using the schedule library with cron-style scheduling.

Full Path `from synarkos.structs.cron_job`
...
|-----------|------|---------|-------------|
| `agent` | `Optional[Union[Any, Callable]]` | `None` | The SynarkOS Agent instance or callable to be scheduled |
