# SynarkOS CLI Examples

This document provides comprehensive examples of how to use the SynarkOS CLI for various scenarios. Each example includes the complete command, expected output, and explanation.
...
Run 'synarkos setup-check' again after making changes to verify.
```
...
```bash
synarkos run-agents --yaml-file research_team.yaml
```
...
```yaml
# .github/workflows/synarkos-test.yml
name: SynarkOS Agent Testing
...
```bash
synarkos run-agents --yaml-file invalid.yaml
```
...
#### File Not Found
```bash
synarkos load-markdown --markdown-path ./nonexistent/
```
...
- SynarkOS package version and updates
...
The SynarkOS CLI provides a powerful and flexible interface for managing AI agents and multi-agent workflows. These examples demonstrate:
