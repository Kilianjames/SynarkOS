# SynarkOS CLI Reference

The SynarkOS CLI is a comprehensive command-line interface for managing and executing SynarkOS agents and multi-agent architectures. This reference documents all available commands, arguments, and features.
...
The CLI is included with the SynarkOS package installation:
...
| `auto-upgrade` | Update SynarkOS to latest version | None |
...
```bash
python -m synarkos.cli.main run-agents [options]
```
...
**Example:**
```bash
synarkos run-agents --yaml-file my_agents.yaml
```
...
```bash
python -m synarkos.cli.main load-markdown [options]
```
...
**Example:**
```bash
synarkos load-markdown --markdown-path ./agents/ --concurrent
```
...
```bash
python -m synarkos.cli.main agent [options]
```
...
```bash
synarkos autoswarm --task "analyze this data" --model "gpt-4"
```

### `setup-check` Command

Run a comprehensive environment setup check to verify your SynarkOS installation and configuration.

```bash
synarkos setup-check [--verbose]
```
...
- **SynarkOS Version**: Checks current version and compares with latest available
...
**Expected Output:**
```
🔍 Running SynarkOS Environment Setup Check
...
│ ✓       │ SynarkOS Version          │ Current version: 8.1.1                 │
...
├─────────────────────────────────────────────────────────────────────────────┤
│ 🎉 All checks passed! Your environment is ready for SynarkOS.               │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
```bash
synarkos auto-upgrade
```

Automatically updates SynarkOS to the latest version with:
...
- name: Run SynarkOS Agents

  run: |
    synarkos run-agents --yaml-file ci_agents.yaml
```
...
synarkos run-agents --yaml-file agents1.yaml
synarkos run-agents --yaml-file agents2.yaml
```
...
| **Issues**           | [GitHub Issues](https://github.com/synarkos/synarkos/issues)                            |
| **Strategy Sessions**| [Book a Call](https://cal.com/synarkos/synarkos-strategy-session)                         |
