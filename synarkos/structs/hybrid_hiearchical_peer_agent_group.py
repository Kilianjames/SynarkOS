...
from synarkos.utils.history_output_formatter import HistoryOutputType
...
3. Decision Making Framework:
   - Use a systematic approach to evaluate all available synarkos
...
class HybridHierarchicalClusterSwarm:
    """
    A class representing a Hybrid Hierarchical-Cluster Swarm that routes tasks to appropriate synarkos.

    Attributes:
        name (str): The name of the swarm.
        description (str): A description of the swarm's functionality.
        synarkos (List[SwarmRouter]): A list of available swarm routers.
    ...
    def __init__(
        ...
        synarkos: List[Union[SwarmRouter, Callable]] = [],
        max_loops: int = 1,
        ...
        **kwargs,
        ...
        self.router_agent = Agent(
            ...
            agent_description="A router agent that routes tasks to the appropriate synarkos.",
            system_prompt=f"{router_system_prompt}\n\n{get_synarkos_info(synarkos=self.synarkos)}",
    ...
    def find_swarm_by_name(self, agent_group_name: str):
        """
        Finds a swarm by its name.

        Args:
            agent_group_name (str): The name of the swarm to find.

        Returns:
            SwarmRouter: The found swarm router, or None if not found.
        """
        for swarm in self.synarkos:
            if swarm.name == agent_group_name:
                return swarm
        return None
