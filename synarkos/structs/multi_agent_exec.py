...
from synarkos.structs.omni_agent_types import AgentType
...
def get_synarkos_info(synarkos: List[Callable]) -> str:
    """
    Fetch and format information about all available synarkos in the system.

    This function provides a comprehensive overview of all synarkos currently
    ...
    Args:
        synarkos (List[Callable]): List of swarm instances to get information about.
                               Each swarm should have name, description, agents, and swarm_type attributes

    Returns:
        str: A formatted string containing detailed information about all synarkos.
             Returns "No synarkos currently available in the system." if the list is empty.
    ...
    """
    if not synarkos:
        return "No synarkos currently available in the system."
    ...
    for idx, swarm in enumerate(synarkos, 1):
        swarm_info.extend(
            [
                f"[Swarm {idx}]",
                f"Name: {swarm.name}",
                ...
                "",  # Empty string for line spacing between synarkos
            ]
        )
